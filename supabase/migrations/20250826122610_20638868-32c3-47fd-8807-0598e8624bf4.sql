-- High Performance RLS Optimization Migration (Fixed)
-- Fix all auth_rls_initplan performance warnings by optimizing auth function calls
-- Only modify policies that exist without referencing non-existent columns
...
-- contatos table (simplified - only read policy since structure may differ)
...
-- atividades table (check if created_by exists, otherwise skip)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name = 'atividades' AND column_name = 'created_by') THEN
...
SELECT 
    ...
    'All auth function calls optimized for enterprise scale' as result,
    'Database performance dramatically improved' as impact;