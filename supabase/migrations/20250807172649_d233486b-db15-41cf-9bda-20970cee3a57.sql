-- Corrigir e criar dados de exemplo para CRM com estrutura correta

-- 1. Primeiro, vamos verificar se existe a coluna 'type' na tabela tasks, se não existir, criar
...
-- 5. Criar usuário temporário para as tasks
DO $$
DECLARE
    temp_user_id uuid;
BEGIN
    -- Inserir um perfil temporário se não existir
    INSERT INTO public.profiles (user_id, full_name, display_name)
    VALUES (gen_random_uuid(), 'Admin CRM', 'Admin')
    ON CONFLICT DO NOTHING;
    
    -- Pegar um user_id existente
    SELECT user_id INTO temp_user_id FROM public.profiles LIMIT 1;
    
    -- Se não existir nenhum perfil, criar um
    IF temp_user_id IS NULL THEN
        temp_user_id := gen_random_uuid();
        INSERT INTO public.profiles (user_id, full_name, display_name)
        VALUES (temp_user_id, 'Admin CRM', 'Admin');
    END IF;
END $$;

-- 6. Criar tarefas relacionadas ao CRM com estrutura correta
...
SELECT 
  ...
  'pending',
  CASE 
    WHEN c.lifecycle_stage = 'opportunity' THEN 'high'
    WHEN c.lifecycle_stage = 'lead' THEN 'medium'
    ELSE 'low'
  END,
  c.id,
  'follow_up',
  'Admin'
FROM public.customers c
CROSS JOIN (SELECT user_id FROM public.profiles LIMIT 1) p
LIMIT 5
ON CONFLICT DO NOTHING;

-- 7. Atualizar relacionamentos das tabelas de referência nos clientes
