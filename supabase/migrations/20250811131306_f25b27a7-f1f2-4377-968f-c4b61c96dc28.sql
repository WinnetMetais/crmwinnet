-- Fix RLS policies: split ALL policies into INSERT/UPDATE/DELETE to avoid multiple permissive SELECT
DO $$
DECLARE
  rec RECORD;
  v_qual text;
...
BEGIN
  ...
  LOOP
    ...
    SELECT 
      pg_get_expr(pol.qual, c.oid, true) AS qual_expr,
      pg_get_expr(pol.with_check, c.oid, true) AS check_expr
    INTO v_qual, v_check
