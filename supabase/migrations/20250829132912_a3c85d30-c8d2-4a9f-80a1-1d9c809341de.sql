-- Criar trigger para deals table
DROP TRIGGER IF EXISTS sync_sales_to_finance_trigger ON public.deals;

CREATE TRIGGER sync_sales_to_finance_trigger
    AFTER UPDATE ON public.deals
    FOR EACH ROW
    EXECUTE FUNCTION public.sync_sales_to_finance();

-- Corrigir função sync_sales_to_finance com search_path
DROP FUNCTION IF EXISTS public.sync_sales_to_finance();
