-- FASE 1: Criar tabela completa de Orders/Pedidos (CORRIGIDA)
...
-- Trigger automático para criar pedido quando orçamento é aprovado
    ...
    IF NEW.status IN ('aprovado', 'aceito', 'approved') THEN
      ...
      IF NOT EXISTS (SELECT 1 FROM public.orders WHERE quote_id = NEW.id) THEN
        ...
        INSERT INTO public.orders (
          ...
          NEW.total,
          'CONFIRMED',
          'Pedido gerado automaticamente do orçamento ' || COALESCE(NEW.quote_number, NEW.id::TEXT),
...
-- Função para automatizar Order → Invoice (CORRIGIDA)
  ...
  IF (TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status) THEN
    IF NEW.status = 'DELIVERED' THEN
      ...
      IF NOT EXISTS (SELECT 1 FROM public.invoices WHERE order_id = NEW.id) THEN
        ...
        -- Copiar itens do pedido para a fatura (CORRIGIDA REFERÊNCIA)
...
DROP TRIGGER IF EXISTS auto_create_invoice_from_order_trigger ON public.orders;
